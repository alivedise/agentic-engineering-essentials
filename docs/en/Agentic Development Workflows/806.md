...
The cost of no quality gates is not just bad output — it is hard-to-diagnose bad output. When a downstream agent produces unexpected results, the question is which upstream stage produced the bad input. Quality gates create fixed points where output has been verified against an explicit contract; this bounds the search space for failures.
...
- Using a different model for evaluation than for production (to avoid correlated failure modes)
...
In a multi-stage pipeline, gates chain: each stage's output gate must pass before the next stage begins. On failure, the gate blocks the pipeline: the output does not advance to the next stage, and the failure details are returned to the producing stage. The failure routing matters:
...
A retry budget is required: without a retry budget, a failing output can cycle the gate loop indefinitely. Define a retry budget limit per gate type; on exhaustion, escalate to a human approval gate (AEE-804).
