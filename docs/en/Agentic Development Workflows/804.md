...
Three oversight patterns map to different positions in a workflow and different risk profiles.
...
- Scope-based and confidence-based interrupt triggers SHOULD be defined per workflow; teams MUST define the scope threshold and confidence floor before deploying agents on consequential tasks.
...
Async review creates a feedback loop only if someone reads the logs. Teams that adopt async review without a defined review cadence (who reviews, how often, what they look for) are not doing async review — they have a log, not an oversight practice.
...
```json
{
  "action": "...",
  "confidence": 0.72,
  "uncertainty_reason": "multiple valid implementations identified; prefer team convention but cannot verify without more context"
}
```

**Harness routing.** The workflow harness reads the confidence field and routes based on defined thresholds. For example, a team might define thresholds as follows: confidence above 0.9 — proceed; confidence 0.7–0.9 — async review; confidence below 0.7 — interrupt point. These values are illustrative. Thresholds are team-specific and domain-specific: in a domain where errors are cheap (draft copy, exploratory analysis), a lower threshold is appropriate; in a domain where errors are expensive (infrastructure configuration, database schema), the threshold should be higher and the default should favor interruption.

**Defined thresholds.** Thresholds must be explicitly configured before deploying agents on consequential tasks. They are not universal standards — they are team policy decisions that depend on the cost profile of the domain.
...
| Agent confidence in mid-range (0.7–0.9, team-defined) | Async review | Uncertain but not blocking; structured log captures reasoning for calibration |
...
1. **Design interrupt points around action type, not agent confidence alone.** Confidence and reversibility are orthogonal axes; treat them separately. A high-confidence agent can still be confidently wrong about an irreversible action. Interrupt on action type first; use confidence thresholds as an additional layer, not a substitute.
