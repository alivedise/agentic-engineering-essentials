...
Agentic workflows MUST include at least one human checkpoint for tasks with irreversible consequences. Steering rules SHOULD be versioned alongside code.
...
At scale, these conditions no longer hold. Across a team, context is distributed -- agents operating with different steering rules tend to produce inconsistent outputs, the kind of integration failures that have no single identifiable cause. Across multiple sessions, lessons not codified are typically rediscovered by the next agent session or the next engineer, at a cost each time. With multiple agents operating in parallel, the surface area of consequential decisions expands faster than any one reviewer can cover.
...
|---|---|---|
| AEE-801 | AI-Driven Development Lifecycle | Three-phase lifecycle (Inception/Construction/Operations), adaptive workflow, steering rules as phase-level behavioral constraints |
...
| AEE-804 | Human Oversight Patterns | Interrupt points, approval gates, async review, confidence thresholds, pattern selection |
...
### Applied Practice Articles

AEE-807 and AEE-808 extend specific methodology concerns with applied-practice depth.

| Article | Topic | What it covers |
|---|---|---|
| AEE-807 | Spec-Driven Development Frameworks in Practice | Survey of OpenSpec, BMAD-METHOD, Kiro, GitHub Spec Kit, Superpowers, Compound Engineering, and get-shit-done; extends AEE-802 |
| AEE-808 | AGENTS.md and Authoring Best Practices | AGENTS.md cross-tool convention, interop with CLAUDE.md, short-vs-comprehensive authoring debate; extends AEE-803 |
