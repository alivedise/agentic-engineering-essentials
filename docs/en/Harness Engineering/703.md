...
The assembly is ordered. The system prompt, which appears as the top-level `system` parameter in the API request, is processed first and carries the highest weight. The messages array follows. Within the messages array, earlier content receives more attention than later content. Getting the order wrong degrades model behavior without any visible error.
...
At each turn, the harness assembles context from five sources. Two are separate top-level API parameters; three live in the messages array:
...
def assemble_context(
    ...
    # Skill injections are appended here, not added as message turns
    system = build_system_prompt(session.persona, session.constraints)
    for skill in active_skills:
        system += "\n\n" + truncate(skill.body, budget["skill_injections"])
    ...
    return {
        "system": truncate(system, budget["system_prompt"] + budget["skill_injections"]),
        "messages": messages,
...
- [AEE-204](../Prompt Engineering/204) -- System Prompt Engineering
