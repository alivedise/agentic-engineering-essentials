...
harness.on("pre_tool", audit_log)            # fires third
```
...
For the full list of event types and handler options, see the [Claude Code Hooks documentation](https://code.claude.com/docs/en/hooks).
...
3. **Keep hooks fast.** A slow pre-tool hook delays every tool call. If a hook needs to do expensive work (remote API call, database write), make it asynchronous or fire-and-forget. Pre-tool hooks that add significant latency will measurably degrade loop throughput.
