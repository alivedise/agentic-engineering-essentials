...
- Session history MUST NOT be shared across user boundaries. Cross-user history leakage is a security failure.
...
- **User ID validation on lookup.** Session lookup MUST validate that the `user_id` in the session record matches the authenticated user. Looking up by session_id alone allows any authenticated user to read any session.
...
- **Secure session token handling.** Session tokens passed to clients MUST be treated as credentials -- transmitted over HTTPS only, stored in HttpOnly cookies or secure client-side storage.
