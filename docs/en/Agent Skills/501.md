...
The core claim: a skill is not a prompt — it is a packaged behavioral contract. It has a name, an invocation condition, a defined scope, and an expected effect on agent reasoning. Engineers who treat skills as ad-hoc instructions accumulate brittle one-offs; engineers who design skills as contracts build reusable systems.
...
4. **Body**: the behavioral guidance itself — the instructions, constraints, and patterns that shape the agent's approach when this skill is active.
