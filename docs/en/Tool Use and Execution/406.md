...
- `--memory=256m`: 256MB memory ceiling
- `--cpus=0.5`: 50% of one CPU
- `--read-only`: root filesystem is read-only
- `--tmpfs /tmp:size=64m`: 64MB writable temp directory
...
Browser automation and computer use tools have the highest blast radius of any tool type — they can interact with any application or website visible in the browser or on screen. Two isolation requirements apply:

**Computer use:** The agent must run inside a dedicated VM or container that is completely separate from the host system. Computer use grants the model control over a mouse, keyboard, and screen — if that screen is the host desktop, the agent can access every application, credential store, and filesystem path available to the logged-in user. Run computer use inside a headless VM (e.g., a cloud VM or a local container with a virtual display).

**Browser automation:** Restrict the browser to a domain allowlist. An agent granted unrestricted web access can exfiltrate data to arbitrary URLs, interact with external services, or be redirected by prompt injection in web content. Confine browser automation to the domains the task requires. For Playwright, this can be enforced at the network layer:
...
# Block all requests not matching the allowed domains
...
These isolation requirements apply regardless of whether the tool uses the Anthropic computer use API, Playwright, or another automation framework. The isolation is at the infrastructure level, not the prompt level.
...
For pre-execution log entries (written before an irreversible action is dispatched), set `result` to `null` and update the entry after execution completes.
...
The `approval_id` parameter enforces the gate at the schema level — the model cannot call `delete_document` without providing an ID it can only obtain by going through the approval flow.

The schema requirement prevents omission — the model cannot call `delete_document` without supplying some value for `approval_id`. However, it does not prevent the model from hallucinating an arbitrary string. The harness must validate the submitted `approval_id` against a store of issued approvals before executing the delete. Schema enforcement and harness enforcement together close the gate; schema enforcement alone does not.
