...
The 2025-11-25 revision also adds a long-lived-SSE close-and-poll pattern absent from 2025-06-18. After the server starts an SSE stream, it **SHOULD** immediately send a primer event with an empty `data` field and an event ID so the client has a resumption marker, and **MAY** then close the connection without terminating the SSE stream itself. Before closing, the server **SHOULD** send a `retry` field with a delay value; the client **MUST** wait that delay before reconnecting. The reconnect itself uses the standard SSE resumability mechanism — an HTTP GET to the same endpoint with a `Last-Event-ID` header so the server can resume from the last delivered event. Both pieces are needed for graceful reconnect across network drops, and together they let servers behind idle-connection killers (load balancers, serverless platforms) drop the TCP connection without forcing the client off the stream.
...
Four failure modes are HTTP-transport-specific. The numbers and details are covered in §Latency, cost, reliability and §Auth tiers; this is the index for a reader who knows the problem is on the HTTP side.

- **Network blip / reconnect storm.** HTTP and SSE clients (Claude Code as the host example) retry with exponential backoff up to five attempts. Servers should handle reconnects gracefully, either replaying buffered events keyed by event ID or accepting gaps. Stream resumption uses the `Last-Event-ID` header on a fresh HTTP GET.
- **Wall-clock and CPU caps on serverless.** AWS Lambda has a hard 15-minute wall-clock cap per invocation; long-held SSE hits it. Cloudflare Workers do not cap wall-clock for HTTP-triggered requests, but Workers do cap CPU time.
- **Cold start.** Serverless first-request latency varies by runtime and platform. Cloudflare Workers cold start is sub-5ms; AWS Lambda for Node.js typically lands in the hundreds of milliseconds. Acceptable for occasional use; perceptible for chatty flows.
- **DNS rebinding.** Covered in §Auth tiers below; servers **MUST** validate the `Origin` header, and local-only HTTP servers **SHOULD** bind to `127.0.0.1`.
