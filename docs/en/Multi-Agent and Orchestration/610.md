...
AG-UI is sometimes confused with A2UI, a separate specification originated by Google and covered in [AEE-611](611). In short: A2UI is a declarative generative-UI payload format that describes what to render, while AG-UI is the bi-directional runtime connection that streams events between an agent and a frontend. The two layers are complementary; A2UI's v0.9 specification explicitly lists AG-UI as one of its transports, so an agent that already speaks AG-UI can drive A2UI without custom integration.
...
5. **Engineers MUST distinguish AG-UI from A2UI when discussing the protocol.** AG-UI is the runtime event-stream protocol; A2UI is Google's declarative generative-UI payload format, covered in [AEE-611](611). They are complementary layers that compose, and should not be conflated (Claim 12).
...
- [AEE-611](611) — A2UI: Google's declarative generative-UI payload format. Composes with AG-UI as transport; the v0.9 spec lists AG-UI as one of A2UI's supported transports.
