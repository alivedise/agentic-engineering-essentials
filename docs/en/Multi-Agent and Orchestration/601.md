...
**When to use:** The task requires agents to actively challenge each other's reasoning, iterate on shared hypotheses, or negotiate outcomes. Debugging scenarios where multiple agents investigate competing hypotheses and debate their findings is a documented use case for this topology. Use this topology only when direct agent collaboration is a specific requirement — the coordination cost is high.
...
**Review loop** is not a base topology — it is a quality-assurance pattern applied on top of supervisor/worker. A worker produces output, a separate reviewer agent evaluates it, and if the review fails, the output is returned to the worker for revision. The loop continues until the reviewer is satisfied or a retry limit is reached. The key principle: the implementer and reviewer must be different agent instances — the same instance evaluating its own output introduces bias and the review fails to catch the original errors.
