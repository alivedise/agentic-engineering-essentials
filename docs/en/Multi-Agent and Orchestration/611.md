...
**Direct JSON for languages without a published SDK.** A2UI ships two official agent SDKs: Python as the reference implementation, published on PyPI as `a2ui-agent-sdk` and depending on `a2a-sdk`, `google-adk`, `google-genai`, and `jsonschema` (Claim 19); and Kotlin. Both are governed by the language-agnostic conformance suite at `agent_sdks/conformance/` (Claim 20). SDK Guide § 10 describes how to port the SDK to other languages, with C++ and Java named as examples, but the repository does not ship implementations in those languages. There is no TypeScript or JavaScript agent SDK in v0.9 either; agents in those languages emit A2UI JSON directly. The spec ships formal JSON Schema (draft 2020-12) under `specification/v0_9/json/` for messages, capabilities, data model, and the basic catalog, so any agent can statically validate output before transmission (Claim 21).
...
- 2026-05-11 — Corrected agent SDK language count: A2UI ships two SDKs (Python as reference, Kotlin), not "five language slots" as prior text claimed; C++ and Java appear only as porting examples in SDK Guide § 10. Removed "Best Practices" section: the protocol is in Public Preview with v0.8 just renamed and v0.10 already in the spec tree — not yet stable enough to call any guidance a "practice"; the content of the removed BPs is already supported in "Design Think", "Surface and Component Model", and "Agent-Side Integration" by direct spec citations. zh-tw version rewritten in parallel for fluency.
