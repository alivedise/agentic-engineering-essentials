...
5. **Lock down the catalog.** The protocol's security stance assumes the agent emits declarative data and is restricted to components from the client's pre-approved catalog (Claim 3, Claim 9). Ship the smallest catalog that covers your use case, review additions to it as security-relevant changes, and treat `basic_catalog.json` as a starting point to harden, not a default to ship as-is.
