...
- All workers must be dispatched before fan-in can begin — the fan-in step cannot begin until the slowest worker completes, blocking aggregation of all results even if other specialists finish first.
...
Agents arranged in sequence: A → B → C → D. Each agent receives the previous agent's output as its primary input. No stage can begin until the previous stage completes and produces an output contract that specifies the format the next stage will receive.
