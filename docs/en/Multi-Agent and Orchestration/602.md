...
Passing too much context to a worker degrades performance. A worker that receives 50,000 tokens of orchestration history when it only needs 2,000 tokens of task context will underperform a worker that receives exactly what it needs. The orchestrator's job is not to forward everything it knows — it is to curate what each worker needs to succeed.
...
When neither condition holds, message passing is the better default.
...
2. **Curate worker context aggressively.** A worker that receives 50,000 tokens of conversation history when it only needs 2,000 tokens of task context will underperform a worker that receives exactly what it needs. Every token added to a worker's context is a decision — include it because the worker needs it, not because it was easy to forward.
...
- Anthropic. "Multi-agent systems." Anthropic Documentation. https://docs.anthropic.com/en/docs/build-with-claude/agents/multi-agent-systems
