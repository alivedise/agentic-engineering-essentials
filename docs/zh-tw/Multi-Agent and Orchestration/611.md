...
A2UI 與傳輸無關。v0.9 規格說明該協定「定義伺服端（Agent）與用戶端（Renderer）之間的 JSON 訊息結構與語意合約，但不指定特定傳輸層」（Claim 5）。在候選傳輸之中，[MCP](../Tool Use and Execution/407)（代理至工具平面）與 Websockets、REST、AG-UI 及 A2A 並列。v0.9 公告以具體用語闡明同一觀點——「A2UI over MCP、Websockets、REST、AG UI、A2A，或任何你想要的傳輸」——並補充「任何已支援 AG-UI 的代理皆能在 day zero 驅動 A2UI v0.9。無需自訂整合」（Claim 6）。這是 A2UI 與 AEE-610 關係最清晰的表述：AG-UI 是 A2UI 所搭乘的傳輸之一，兩個協定設計上即可組合。
...
A2UI 的代理端負責發出訊息。`google/A2UI` 儲存庫為此提供三層官方工具，其中建議的整合模式記載於 Agent SDK Guide。

**透過 `SendA2uiToClientToolset` 進行 tool calling。** 建議的整合模式採用 LLM function calling。SDK 提供一組 toolset，其中 `send_a2ui_json_to_client` 工具由 LLM 呼叫以推送 UI 更新（Claim 17）。代理程式不直接撰寫 A2UI JSON；LLM 推理該渲染哪份 UI、呼叫該工具，由 SDK 將呼叫翻譯為合法的 A2UI 訊息再送出。此模式記載於 `agent_sdks/agent_sdk_guide.md` 的第 9 節，Python ADK 實作位於 `a2ui.adk.send_a2ui_to_client_toolset`。

**搭配文字界定符的串流解析模式。** 第二種互補模式處理 tool calling 不可用、或代理輸出文字補全的情境。LLM 將 A2UI JSON 包在如 `<a2ui-json>` 之類的界定符中，置於其常規文字串流內；SDK 的 `A2uiStreamParser` 逐 chunk 消化串流，在完整訊息抵達時逐增量產出 A2UI Parts（Claim 18）。通訊以 JSON 物件串流的形式漸進解析，conformance suite 在語言實作之間強制其在 chunk 緩衝與如截斷 token 等邊緣情境上的一致性（Claim 24）。

**沒有官方 SDK 的語言可直接發出 JSON。** 代理 SDK 原始碼共有五個語言位置——Python、Kotlin、Java、C++——統一受語言中立的 conformance suite 約束（Claim 20）。Python SDK 以 `a2ui-agent-sdk` 名稱發布於 PyPI，相依於 `a2a-sdk`、`google-adk`、`google-genai` 與 `jsonschema`（Claim 19）。v0.9 並無 TypeScript 或 JavaScript 代理 SDK；上述語言的代理直接發出 A2UI JSON。規格在 `specification/v0_9/json/` 下提供 JSON Schema（draft 2020-12），涵蓋訊息、capabilities、資料模型與 basic catalog，因此任何代理皆可在傳送前對輸出進行靜態驗證（Claim 21）。

**Session 起始的 catalog 交涉。** Catalog 選擇是執行期協商。用戶端透過傳輸層的 metadata 或初始化載荷（A2A metadata、Agent Cards、MCP initialization 或 AG-UI 事件）發送 `a2uiClientCapabilities.supportedCatalogIds`（catalog URI 清單）以及可選的 `inlineCatalogs`，代理便將其元件選擇限制於這些 catalog 之中（Claim 22）。此交涉取代了較早的「雙方在編譯期就 catalog 達成協議」模型，那是部分早期文章常隱含的假設。

**參考代理。** `samples/agent/adk/` 下的八個可執行 Python 代理示範了端到端的模式：`restaurant_finder`、`orchestrator`、`personalized_learning`、`rizzcharts`、`gemini_enterprise`、`custom-components-example`、`mcp-apps-in-a2ui-sample` 與 `mcp_app_proxy`（Claim 23）。皆以 Google ADK 專案形式發布；對新整合者最容易上手的入口是同時閱讀 `restaurant_finder` 與 Agent SDK Guide。
...
5. **鎖緊 catalog。** 該協定的安全立場假設代理發出宣告式資料且受限於用戶端預先核可的 catalog 中元件（Claim 3、Claim 9）。出貨涵蓋你使用情境的最小 catalog，將其中新增項目視為與安全相關的變更加以審查，並將 `basic_catalog.json` 視為待強化的起點而無需原樣出貨。Catalog 選擇於執行期透過 `a2uiClientCapabilities.supportedCatalogIds` 進行（Claim 22），因此伺服器接受的 catalog 清單是部署期設定，應與其他安全政策納入同一審查軌道。
...
- 2026-04-28 — 新增「代理端整合」章節，說明 SDK + tool calling、串流解析器、JSON Schema、catalog 交涉與參考代理；同步更新 EN 版本與 BP 5。
