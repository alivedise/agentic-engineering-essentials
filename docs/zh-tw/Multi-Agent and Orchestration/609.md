...
3. **將 ACP 與 MCP 視為互補層次。** ACP 將自身定位為與 Anthropic 的 [MCP](../Tool Use and Execution/407) 互補。IBM 採用的框架是「MCP connects agents to their tools and knowledge」（MCP 連接代理至其工具與知識），而「ACP connects agents to agents」（ACP 連接代理至代理）。架構決策 SHOULD 將兩者視為代理堆疊中的不同層次。同樣的邏輯延伸至作為後繼者的 A2A。
