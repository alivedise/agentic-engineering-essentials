...
AG-UI 是 agent-to-user-interface 軸線，與 A2A（agent-to-agent）和 [MCP](../Tool Use and Execution/407)（agent-to-tool）並列為第三個正交的協定平面。MCP 標準化代理如何接觸工具，A2A 標準化代理如何接觸其他代理，AG-UI 則標準化代理執行中的活動如何抵達面向人類的應用程式，包括聊天面板、儀表板、IDE 側邊欄，以及周邊 UI 介面，使其能即時呈現代理正在做什麼。
...
AG-UI 有時會與 A2UI 混淆，後者是由 Google 提出的另一份規範，已在 [AEE-611](611) 詳述。簡而言之：A2UI 是描述要渲染什麼的宣告式生成式 UI 載荷格式，AG-UI 則是雙向執行期連線，在代理與前端之間串流事件。兩個層次互補；A2UI v0.9 規範明確列出 AG-UI 為其支援的傳輸方式之一，因此已會說 AG-UI 的代理可直接驅動 A2UI，無需客製化整合。
...
5. **工程師 MUST 在討論該協定時將 AG-UI 與 A2UI 區分清楚。** AG-UI 是執行期事件串流協定；A2UI 是 Google 的宣告式生成式 UI 載荷格式，已在 [AEE-611](611) 詳述。兩者是可組合的互補層次，且不應混為一談（Claim 12）。
...
- [AEE-611](611) — A2UI：Google 的宣告式生成式 UI 載荷格式。與 AG-UI 作為傳輸層相互組合；A2UI v0.9 規範將 AG-UI 列為其支援的傳輸方式之一。
