...
harness.on("pre_tool", audit_log)            # 第三觸發 — 如果 check_permissions 拋出異常，永遠不會觸發
```
...
3. **保持鉤子快速。** 緩慢的前置工具鉤子會延遲每次工具呼叫。若鉤子需要執行昂貴的操作（遠端 API 呼叫、資料庫寫入），應將其設計為非同步或即發即忘。在每次呼叫上增加超過約 50ms 的前置工具鉤子，將可量測地降低迴圈吞吐量。
