...
組裝是有順序的。系統提示詞作為 API 請求中的頂層 `system` 參數出現，最先被處理，具有最高的權重。messages 陣列緊隨其後。在 messages 陣列中，越早出現的內容獲得越多的注意力。順序錯誤會在沒有任何明顯錯誤的情況下使模型行為退化。
...
在每一輪中，harness 從五個來源組裝情境。其中兩個是獨立的頂層 API 參數；三個存在於 messages 陣列中：
