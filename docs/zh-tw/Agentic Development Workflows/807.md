...
**再生模型（regeneration model）** — 當需求改變時，會發生什麼？調查中出現兩種哲學。規格作為真實來源（spec-as-source-of-truth）框架（OpenSpec、GitHub Spec Kit、get-shit-done）將規格工件視為持久的真實，並將變更串聯向下傳遞：更新規格、重新產生計畫、重新產生任務。規格作為輸入（spec-as-input）框架（BMAD-METHOD、Superpowers；Kiro 為推斷）將規格視為一次性生成週期的輸入；改變需求意味著從更早的階段重新進入工作流程。再生模型決定了中途變更需求所觸發的返工量。
...
BMAD-METHOD（「突破式敏捷 AI 驅動開發方法」）是一個代理式敏捷（agentic agile）框架，將具名的 AI 人格對映到敏捷角色，引導專案從初始想法到部署。
...
**AEE 對應。** 完整專案生命週期覆蓋，透過品質閘門對映到 AEE-801（AI-DLC 建構階段）。人格結構與 AEE-603（任務分解與委派）一致：每個具名代理是一個具有有限職責的專門子代理。
...
**AEE 對應。** 直接實作 AEE-802 規格原則：三工件結構對映到目標陳述（requirements）、架構（design.md）和任務分解（tasks.md）。Kiro IDE 的工作階段層級引導規則（steering rules）強制執行 AI-DLC 慣例——這是 AEE-803 原則應用於工具層的一個實例。
...
|---|---|---|---|---|
| OpenSpec | 能力層級差異提案 | 持續變更管理 | 實作前（validate）、實作中（任務審查）、實作後（歸檔） | 規格即真實來源（spec-as-source-of-truth）；archive 持續累積 |
| BMAD-METHOD | PRD → 架構 → 史詩 / 故事串聯 | 每史詩持續進行 | 每個人格移交時 | 規格作為輸入；代理鏈 |
| Kiro AI-DLC spec mode | 每功能多階段（需求 → 設計 → 任務） | 持續進行（實作後更新規格） | 階段轉換時 | 規格作為輸入（推斷） |
| GitHub Spec Kit | 每功能多階段（specify → plan → tasks） | 每功能一次性 | 斜線指令之間 | 規格即真實來源（spec-as-source-of-truth）；迭代精煉 |
