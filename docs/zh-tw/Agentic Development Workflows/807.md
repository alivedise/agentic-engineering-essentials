...
**再生模型（regeneration model）** — 當需求改變時，會發生什麼？調查中出現兩種哲學。規格作為真實來源（spec-as-source-of-truth）框架（OpenSpec、GitHub Spec Kit、get-shit-done；Kiro 為推斷）將規格工件視為持久的真實，並將變更串聯向下傳遞：更新規格、重新產生計畫、重新產生任務。規格作為輸入（spec-as-input）框架（BMAD-METHOD、Superpowers）將規格視為一次性生成週期的輸入；改變需求意味著從更早的階段重新進入工作流程。再生模型決定了中途變更需求所觸發的返工量。
