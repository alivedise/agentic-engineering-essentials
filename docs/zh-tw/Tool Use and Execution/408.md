...
2025-11-25 還新增了一套 2025-06-18 沒有的長連線 SSE 模式。伺服器開啟 SSE 串流後，**SHOULD** 立刻送出一筆事件，事件 ID 一併帶上、`data` 欄位留空，讓用戶端先取得重連用的標記；接著 **MAY** 在不終止 SSE stream 本身的情況下關掉這條連線。關閉前 **SHOULD** 送出 `retry` 欄位；用戶端 **MUST** 遵守該欄位指定的等待時間，再帶 `Last-Event-ID` 重新連回來。這個模式讓位於閒置連線殺手（負載平衡器、無伺服器平台）後方的伺服器，可以主動斷掉 TCP 連線，又不逼用戶端放棄整條 stream。
