...
瀏覽器自動化 (browser automation) 與電腦使用工具在所有工具類型中爆炸半徑最大——它們能與瀏覽器或螢幕上可見的任何應用程式或網站互動。兩項隔離要求如下：

**電腦使用：** 代理必須在與主機系統完全分離的專用 VM 或容器內執行。電腦使用讓模型能控制滑鼠、鍵盤與螢幕——若該螢幕就是主機桌面，代理即可存取登入使用者所能使用的所有應用程式、憑證儲存庫與檔案系統路徑。應在無頭 VM（例如雲端 VM 或具有虛擬顯示器的本機容器）中執行電腦使用工具。

**瀏覽器自動化：** 將瀏覽器限制在允許網域清單 (domain allowlist) 內。被授予不受限制網路存取的代理可將資料外洩至任意 URL、與外部服務互動，或被網頁內容中的提示詞注入攻擊所重新導向。將瀏覽器自動化限制在任務所需的網域範圍內。對於 Playwright，可在網路層強制執行：
...
無論工具使用的是 Anthropic 電腦使用 API、Playwright 或其他自動化框架，這些隔離要求均適用。隔離是在基礎設施層級實施，而非提示詞層級。
...
對於執行前的稽核日誌（在不可逆操作分發前寫入），將 `result` 設為 `null`，並在操作執行完成後更新該欄位。
...
Schema 層的 `required` 設定可防止參數遺漏——模型在呼叫 `delete_document` 時必須提供 `approval_id`。然而，這無法阻止模型憑空捏造一個任意字串。執行層（harness）必須在真正執行刪除之前，對傳入的 `approval_id` 進行驗證，確認它確實存在於已核發的審批記錄中。Schema 層的強制執行與執行層的驗證相互配合，才能真正關閉此閘道；單靠 Schema 層的設定是不夠的。
