...
Mario Zechner 的 [MCP vs CLI](https://mariozechner.at/posts/2025-08-15-mcp-vs-cli/) 基準測試讓畫面變得更複雜。在同一份底層能力之下，分別以 MCP 伺服器與 CLI 兩種形式公開、跑同一份基準任務：兩者皆達成 100% 成功率；MCP 版本快了 23%，便宜 2.5%。Zechner 的結論其實取決於設計品質：「MCP vs CLI 真的是打成平手」，前提是兩者設計都做好。良好的 MCP 工具設計（schema 明確、回傳結構化、範圍合理）能透過減少命令偵測往返，吸收掉每輪 schema 額外負擔的多數成本。
...
- [AEE-401](401) — 函式呼叫：MCP 路由的工具呼叫信封
- [AEE-402](402) — 工具 schema 設計：套用於 MCP 工具的 schema 規範
- [AEE-405](405) — 工具選擇：模型如何在已宣告的工具之間挑選
- [AEE-408](408) — 本地與遠端 MCP 部署：傳輸、auth、延遲、成本、故障模式
- [AEE-608](../Multi-Agent and Orchestration/608) — A2A：Agent2Agent 協定
- [AEE-609](../Multi-Agent and Orchestration/609) — ACP：代理通訊協定
- [AEE-610](../Multi-Agent and Orchestration/610) — AG-UI：代理與使用者介面互動協定
- [AEE-611](../Multi-Agent and Orchestration/611) — A2UI：代理用宣告式生成介面協定
- [AEE-3](../AEE Overall/3) — 代理工程的層級（Level 5 框架）
