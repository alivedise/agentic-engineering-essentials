...
| skill | A named, bounded capability exposed to an agent via the harness — typically a function or tool the agent can invoke to accomplish a specific subtask (e.g., "web_search", "run_code", "send_email") | https://composio.dev/content/ai-agent-tool-calling-guide |
...
| system prompt | The privileged, model-level instruction block prepended to every conversation; sets persona, rules, available tools, output format, and safety constraints; invisible to the end user but read by the model before any user turn | https://www.salesforce.com/blog/agentic-ai-definitions/ |
...
| fine-tuning | Continued training of a pre-trained base model on a task-specific or domain-specific dataset to specialize its outputs; updates model weights (unlike RAG, which injects knowledge at inference); common subtypes include supervised fine-tuning (SFT), RLHF, and RLVR | https://labelstud.io/blog/reinforcement-learning-from-verifiable-rewards/ |
...
- [SWE-bench Leaderboard](https://www.swebench.com/) — Live benchmark tracking coding-agent performance on real GitHub issues; as of early 2026, top agents (e.g., Verdent) reach 76–81% on SWE-bench Verified; Claude Opus 4.5 was the first model to break 80% (80.9%); SWE-bench Pro scores are ~23% for top models, signaling headroom

- [Top Agentic AI Frameworks in 2025 (Medium / Data Science Collective)](https://medium.com/data-science-collective/top-agentic-ai-frameworks-in-2025-which-one-fits-your-needs-0eb95dcd7c58) — Categorizes 2025 frameworks into four groups: General-Purpose Orchestrators (LangChain, LlamaIndex, Haystack, Smolagents), Workflow/Graph-Based (LangGraph, n8n, Prefect), Multi-Agent Collaboration (AutoGen, CrewAI, MetaGPT, OpenAI Swarm), and Enterprise/Vendor-Specific (Azure AI Agent Service, AWS Bedrock Agents, Google Vertex AI Agent Builder)
